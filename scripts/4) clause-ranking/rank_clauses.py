...
from hybrid_ranker import rank_clauses, rank_clauses_with_diversity
...
from hybrid_ranker import DEFAULT_WEIGHTS
...
def load_gnn_model_and_tokenizer(model_path: str, device: str = "cuda"):
    """
    ...
    # Load InCaseLawBERT
    print("📥 Loading InCaseLawBERT...")
    tokenizer = AutoTokenizer.from_pretrained("law-ai/InCaseLawBERT")
    bert_model = AutoModel.from_pretrained("law-ai/InCaseLawBERT")
...
def process_all_graphs(
    ...
    for graph_file in tqdm(graph_files, desc="Ranking clauses"):
        try:
            ...
            # Save to file
            output_file = output_dir / f"{doc_id}_ranked.json"
