...
INPUT_ROOT = Path("/kaggle/input/your-dataset-name")
OUTPUT_ROOT = Path("/kaggle/working")
PDF_DIR = INPUT_ROOT / "Supreme Court"
SUMMARY_JSON_DIR = INPUT_ROOT / "abstracted_summaries"
OUTPUT_DIR = OUTPUT_ROOT / "final_summary_pdf"

# Prefer pipeline meta_splitter
try:
    from preprocess.meta_splitter import split_metadata_body
except ImportError:
    META_SPLITTER_PATH = INPUT_ROOT / "3_meta_splitter_judge.py"
    if META_SPLITTER_PATH.exists():
...
def find_split_via_ocr(doc):
    ...
    Returns (split_index, split_page, page_texts, meta, y_crop).
    """
    if not _OCR_AVAILABLE:
...
def _max_chars_that_fit(
    ...
    insert_textbox returns >= 0 when text fits. Returns n.
    """
    if not text.strip():
        return 0, None
    temp_doc = fitz.open()
...
def _draw_summary_separator(page, page_rect, y_after_meta, margin_h=50):
    """
    Draw a horizontal line to separate metadata from the summary.
...
def append_summary_to_page(
    ...
    If text overflows, add new pages.
    """
    ...
    min_height = 60
    if start_y >= page_rect.height - margin_b - min_height:
    ...
    if summary_rect.height < min_height or summary_rect.width < 20:
        current_page = out_doc.new_page(width=page_rect.width, height=page_rect.height)
        y_cursor = margin_l
        summary_rect = fitz.Rect(
            margin_l,
...
def crop_pdf_after_metadata_and_append_summary(
    ...
    if y_crop and y_crop < rect.height:
        ...
        # Append BART summary just below metadata
        if summary_text:
...
def process_supreme_court_dir(pdf_dir=None, summary_dir=None, out_dir=None):
    ...
    for i, json_path in enumerate(json_files, 1):
        ...
        output_path = out_dir / f"{stem}.final.pdf"
